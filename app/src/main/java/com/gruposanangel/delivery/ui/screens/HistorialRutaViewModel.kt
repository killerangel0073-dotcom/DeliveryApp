...
class HistorialRutaViewModel(
    ...
    fun cargarDemoData() {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            for (i in 0 until 50) {
                ...
                // Velocidad variable: Aceleración progresiva y picos de exceso
                val vel = when {
                    i in 15..20 -> 98.0 // Tramo de exceso de velocidad (Autopista)
                    i in 35..40 -> 5.0  // Tramo lento (Tráfico/Zona escolar)
                    else -> 45.0 + (kotlin.math.sin(t) * 15.0) // Velocidad crucero
                }
                ...
                puntosDemo.add(
                    PuntoHistorial(
                        ...
                        vel = vel,
                        bat = (100 - (i * 0.5)).toInt(),
                        ts = Timestamp(ahora + (i * 60), 0), // Punto cada minuto
                        acc = 5f,
    ...
    private fun cargarHistorial() {
        ...
        _uiState.update { it.copy(isLoading = true, puntos = emptyList(), incidencias = emptyList(), error = null) }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (doc.exists()) {
                    ...
                    _uiState.update { it.copy(isLoading = false, puntos = emptyList(), error = "No hay registros para este día.") }
                }
            } catch (e: Exception) {
